/**
 * Copyright 2009-2012 StubHub, Inc. All rights reserved.
 */
...
;(function($){
	...
	$.fn.bubblehelpOverlay = function(options) {
		var defaults = {
			width:250,
			trigger:'mouseover',
			offsetX:($.browser.msie && $.browser.version=='8.0')? -10 : -5,
			offsetY:0,
			preventClickDefault:true,
			exclusive:false,
			restoreFocusOnActiveElement:false
		};
		var opts = $.extend(defaults, options || {});
		return this.each(function() {
			$(this).overlay({
	...
	$.fn.james = function (url_to_call, options) {
	        ...
	        if (value_to_send.length > 0 && (o.minlength === false || value_to_send.length >= o.minlength)) {
	            ...
	            $.ajax({
	                ...
	                success:    function (data) {
	                    var arr = o.onKeystroke(data),len = arr.length;
	                    results_set = [];
	                    current_hovered = 0;
	                    for (var i = 0; i < len; i++) {
	                        if (arr[i] !== null) {
	    ...
	    var updateDom = function() {
	    	...
	    	var is_empty = true, len = results_set.length;
...
(function($) {
	$(function() {
		...
		if ($.livequery) {
			$(".helpBubble").livequery($.stubhub.helpBubbleWrapper);
...
(function($, SH) {
	...
	/* 2013-02-04
	 * Refactor the domainRedirect plugin,
	 * Now we bind the live click event to the a[href^="http"] element,
	 * this will filter out the <a href="mailto:mapcorrection@stubhub.co.uk"></a>,
	 * also will filter out the <a href="javascript:;"></a> and so on.
	 */
	$('a[href^="http"]').live('click',function(e){
		
		function isDomainRedirect(url, href){
			var urlSub = url.split('.')[1],
				link = href,
				urlExtension = '',
				compareUrlExtn = '',
				sIndex = link.indexOf('//'),
				eIndex = link.indexOf('/',sIndex+2) > -1 ? link.indexOf('/',sIndex+2) : link.length,
				hostname = link.substring(sIndex+2, eIndex);
			if(hostname.split(".")[1] != urlSub || $('#redirectOverlay').size() == -1 || hostname.split(".")[1] == "stubhubstatic"){
				return false;
			}
			else{
				sIndex = url.indexOf('.');
				eIndex = url.indexOf('.',sIndex+1);
				urlExtension = url.substring(eIndex);
				sIndex = hostname.indexOf('.');
				eIndex = hostname.indexOf('.',sIndex+1);
				compareUrlExtn = hostname.substring(eIndex);
				return urlExtension != compareUrlExtn ? true : false;
			}
		};
		
		var $target = $(this),
			isRedirect = isDomainRedirect(location.hostname, $target.attr('href'));
		
		if(isRedirect){
			e.preventDefault();
			if($target.hasClass('crossDomainContinue')){
				$('#cancelBtn').click();
			...
			$target.overlay({
				...
				open: function(elem){
					...
					$("a.crossDomainContinue").attr("href", hrefValue);
				}
			});
		}
     });
   ...
   $.extend(
      ...
      {
         /*
          * Compare the given domain url with current browser location to see if they are same domain.
          * @param {string} theDomainUrl e.g. www.srwd00.co.uk
          * @return {boolean} ture if same domain
          */
         isSameDomain : function (theDomainUrl) {
            var surl = theDomainUrl ? "" + theDomainUrl : "",
                ext  = surl.split(".")[2],
                currentExt = location.hostname.split(".")[2];
            return !! ext && !! currentExt && ext === currentExt;
         },

         isUKDomain : function () {
            return document.domain.indexOf("uk") >= 0;
         }});
...
/**
 * Overlay configuration for Sell Similar popup.
 * The config could be access via <code>$.stubhub.sell.sellSimilarOverlay</code>
 * This is a reused component at "Sell Thank You" and "MyAccount - Listing/Sales" pages.
 */
(function ($, SH) {

    function getUrlExtension () {
		return location.host.split(".")[2];
    }

	/**
      * Is a GA (General Admission) ticket.
      * @param Seats information in string.
      * @return True if is a GA ticket.
      */
    function isGeneralAdmissionTicket (seatsStr) {
        var s = seatsStr ? $.trim(seatsStr) : "";
        return !s || s === "GA" || s === "General Admission";
	}

    function datePattern(UTCFormat){
        var utcDate = UTCFormat || "";
        utcDate = utcDate.substring(0, 19);
        return new Date((utcDate).replace(/-/g, "/").replace(/[TZ]/g, " "));
    }

    function findDescriptionSpan (source, inputId) {
        return source.find(inputId).next('label').find('span');
    };

    function displayTicketInfoToOverlay (sellSimilarData) {
        var sellSimilarConf     = $('#sellSimilarConf'),
            titleHasPlaceHolder = $('#sellSimilarConf h3').text(),
            sameEventRadioS = '#sameEvent',
            today = (function (dateVal) { return dateVal ? datePattern(dateVal) : new Date(); })($("#sysCurrDate").val()),
            sectionRowHtml = "<b>Section</b> " + sellSimilarData[6] + " | <b>Row</b> " + sellSimilarData[7],
            seatsHtml = isGeneralAdmissionTicket(sellSimilarData[8])
                        ? ""
                        : " | <b>Seats</b> " + $.stubhub.ellipsisLongWords(sellSimilarData[8], 16);

        sellSimilarConf.find('h3').text(titleHasPlaceHolder.replace("{0}", sellSimilarData[3]));
        findDescriptionSpan(sellSimilarConf, sameEventRadioS).text(sellSimilarData[4] + " | " +  sellSimilarData[5]);
        findDescriptionSpan(sellSimilarConf, '#sameSeats').html(sectionRowHtml
        	                                                    + seatsHtml
                                                                + " | " +  sellSimilarData[5]);
        if(datePattern(sellSimilarData[10]) < today) {
            var sameEventRadioInput = $(sameEventRadioS + ":input:radio").parent();
            if(sameEventRadioInput) {
                sameEventRadioInput.hide();
            }
            $("#sameSeats:input:radio").attr("checked", "checked");
        }
    };

    function bindContinueBtnHandler (sellSimilarData) {
        $("#sellSimilarConf .btn-continue").unbind('click');
        $("#sellSimilarConf .btn-continue").bind('click', function() {
            var MYA = MYA || {},
                sellDomainUrl = $.stubhub.sellDomainUrl || MYA.sellDomainUrl;
            if($('#sellDomainUrl').val()){
                sellDomainUrl = $('#sellDomainUrl').val();
            }

            var url = null;

            if ($("#sameEvent:input:radio").attr("checked")) {
                url = sellDomainUrl + "/sell/?eventId=" + sellSimilarData[2];
            } else if ($("#sameSeats:input:radio").attr("checked")) {
                url = sellDomainUrl + "/sell/findevent/?genreId=" + sellSimilarData[0] + "&sellSimilarId=" + sellSimilarData[1];
            } else {
                url = sellDomainUrl + "/sell/findevent/?genreId=" + sellSimilarData[0] + "&eventId=" + sellSimilarData[2];
            }

            if(url != null && sellDomainUrl){
                var checkSellDomain = sellDomainUrl.split(".");
                if (checkSellDomain.length >=3 && getUrlExtension() != checkSellDomain[2]) {
                    $('<a href="'+url+'"></a>').click();
                } else {
    ...
    SH.sell || (SH.sell = {});
    SH.sell.sellSimilarOverlay = {
        modal: true,
        cache: false,
        showCloseIcon: true,
        width: 400,
        closeSelector: '.cancelLink',
        open: function(elem) {
            var $elem = $(elem),
            	sellSimilarData = $elem.attr("name").split('?');

            $('#sellDomainUrl').val($elem.attr("sellDomainUrl"));
            displayTicketInfoToOverlay(sellSimilarData);
            bindContinueBtnHandler(sellSimilarData);
        },
        trigger: 'load',
        source: '#sellSimilarConf'
    };
...
// ===============================================================
// Additional Extention to `$.stubhub`
// ===============================================================
...
;(function ($, SH) {
   ...
   $.extend(SH,
      {
         /**
          * Enable Sprite Style Button.
          * @param elem {jQueryObject} the Element that has Sprite button style.
          * @param typeInput {enum[small, medium, large]} {default[medium]}
          * @param elem return itself.
          */
         enableSpriteBtn: function (elem, typeInput) {
            var btn = $(elem),
                type = typeInput ? typeInput : 'medium';
            btn.removeAttr("disabled");
            btn.removeClass("btn-sprite-" + type + "-disable");
            btn.find('span').removeClass("btn-sprite-" + type + "-text-disable");
            return btn;
         },

         /**
          * Disable Sprite Style Button.
          * @param elem {jQueryObject} the Element that has Sprite button style.
          * @param typeInput {enum[small, medium, large]} {default[medium]}
          * @param elem return itself.
          */
         disableSpriteBtn: function (elem, typeInput) {
            var btn = $(elem),
                type = typeInput ? typeInput : 'medium';
            btn.attr("disabled", "disabled");
            btn.addClass("btn-sprite-" + type + "-disable");
            btn.find('span').addClass("btn-sprite-" + type + "-text-disable");
            return btn;
         },

         enableSmallSpriteBtn: function (elem) {
            return this.enableSpriteBtn(elem, "small");
         },

         disableSmallSpriteBtn: function (elem) {
            return this.disableSpriteBtn(elem, "small");
         },

         enableMediumSpriteBtn: function (elem) {
            return this.enableSpriteBtn(elem, "medium");
         },

         disableMediumSpriteBtn: function (elem) {
            return this.disableSpriteBtn(elem, "medium");
         },

         enableLargeSpriteBtn: function (elem) {
            return this.enableSpriteBtn(elem, "large");
         },

         disableLargeSpriteBtn: function (elem) {
            return this.disableSpriteBtn(elem, "large");
         },

         ellipsisLongWords : function(origText, max_char){
            var re = origText ? "" + origText : "";
		    if (origText.length > max_char) {
			   return origText.substr(0, max_char - 3) + "&#8230;";
		    } else {
			   return origText;
		    }
         }
 ...
 */
;(function ($, SH) {
   SH.dateUtil || (SH.dateUtil = {});
   $.extend($.stubhub.dateUtil,
      {
         currentDate : function () {
            var todayTxt = $('#sysCurrDate').val();
            return todayTxt ? new Date(todayTxt) : new Date();
         },

         isPast : function (dateStr) {
            return new Date(dateStr) <= this.currentDate();
         },

         domainDateFormatLong : function() {
            return SH.isUKDomain() ? 'dd/mm/yyyy' : 'mm/dd/yyyy';
         },

         domainDateFormat : function() {
            return SH.isUKDomain() ? 'dd/mm/yy' : 'mm/dd/yy';
         },

         /**
          * @see DatePicker.formatDate
          */
         domainDatePickerFormat : function() {
            return SH.isUKDomain() ? 'dd/mm/y' : 'mm/dd/y';
         },

         diffDays : function (xDate, yDate) {
            var x = new Date(xDate),
                y = new Date(yDate);
            return Math.round((Math.abs(x.getTime() - y.getTime())) / (1000 * 60 * 60 * 24));
         }
   ...
   $.extend($.Array,
      {
         /**
          * @param n {Integer}
          * @return [0..n-1], otherwise [] when `n` is invalid.
          */
         range: function (x, startInput, stepInput) {
            var re = [],
                step = stepInput ? stepInput : 1,
                n = parseInt(x),
                i = startInput ? startInput : 0;
            while (i < n) {
               re.push(i);
               i += step;
            }
         ...
         /**
          * xs `intersect` (xs ++ ys) == xs
          */
         intersect : function intersect (xs1, ys1) {
            var xs = $.makeArray(xs1),
                ys = $.makeArray(ys1);
            ...
            return $.map(xs, function (x) {
               if(ys1.indexOf(x) >= 0) {
                  ...
                  // return undefined will be ignored in the result list;
                  return undefined;
               }
            });
         }
   ...
   $.extend($.Object,
      {
         ...
         /**
          * {@see jQuery.grep} for List but this is for object.
          */
         grep: function (obj, fn) {
            var re = {};
            if(obj) {
               $.each(obj, function (k, v) {
                  if ($.isFunction(fn) && fn(k, v)) {
                     re[k] = v;
                  }
               });
            }
   ...
   if (!Function.prototype.bind ) {
      Function.prototype.bind = function( obj ) {
         if(typeof this !== 'function') {
            throw new TypeError('Function.prototype.bind - Only target for Function.');
         }
         var slice = [].slice,
...
/* skip alert plugin
 * version 0.1 (27-Sep-2012)
 *
 * This plugin enables confirm functionality before nevigate to another page based on Overlay plugin and jQuery.
 * It provides a comfirm overlay before a skip. If confirms yes, page relocation; if cancel, page abort relocation.
 *
 * fn: (controler)
 * function returns ture or false
 *
 * Options: (optional, extend default overlay options)
 * same usage as overlay plugin
 *
 * Example:
 * $('#helplinks a').skipConfirm(function () { return true;});
 * $('#helplinks a').skipConfirm(function () { return true;},{ width: 500});
 */
(function($){
   var defaults = {
      width: 300,
      modal: true,
      trigger: "load",
      source: "#skipConfirmOverlay",
      showCloseIcon: false,
      closeSelector: ".btn-no"
   },

   genOverlayOpenFn = function (target, clickHandler) {
      var fn = $.isFunction(clickHandler) ? clickHandler : defaultYesClickHandler;
      return function _skpiConformOverlayOpen_ () {
         $(".btn-yes").bind("click", function (event) { fn(target); });
      };
   },

   defaultYesClickHandler = function (target) {
      var url = $(target).attr('href');
      window.location.href = url;
   },

   buildOverlayOpt = function (options, openFn) {
      return $.extend({}, defaults, options || {}, { open: openFn });
   };

   /**
    * @param checkCallback {function :: Boolean} Stop event if return true otherwise contiuse.
    * @param yesCallback {function} {optional} A function which will be invoke at click "Yes" btn.
    *                    It first parameter is the target where click event happened before popup overlay.
    * @param options {object} {optional} Optional Overlay config.
    */
   $.fn.skipConfirm = function (checkCallback, yesCallback, options) {
      this.bind("click",
                function _skipConformOverlay_ (event) {
                   var that = $(this),
                       openFn = genOverlayOpenFn(that, yesCallback),
                       opts = buildOverlayOpt(options, openFn);
                   if ($.isFunction(checkCallback) && checkCallback(that)) {
                      that.overlay(opts);
                      // stop immediate propagation will stop all other handlers afterwards.
                      // check jquery src code for detail.
                      // TODO: double check whether put `e.preventDefault()` at first line works here.
